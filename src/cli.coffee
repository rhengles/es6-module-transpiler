...
class CLI
  ...
  argParser: (argv) ->
    optimist(argv)
      ...
      .options(
        ...
        (args) -> not args.indent or /^(\d|t(|ab))$/i.test(args.indent)
      )
  ...
  processFile: (filename, options) ->
    @fs.readFile filename, 'utf8', (err, input) =>
      ...
      output = @_compile input, moduleName, options.type, coffee: ext is '.coffee', imports: options.imports, indent: options.indent
